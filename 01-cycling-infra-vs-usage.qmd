...
get_city_metrics <- function(city_name, country_name) {
  ...
  if (file.exists(osm_path)) {
    ...
    # Use bounding box for robust retrieval via ogr2ogr, then clip to polygon in R
    bbox <- sf::st_as_sfc(sf::st_bbox(poly))
    
    # Temporarily disable S2 to avoid "loop" errors with osmactive/geos
    sf::sf_use_s2(FALSE)
    osm_city <- tryCatch({
      # We use place = country_name to ensure correct PBF selection
      # We use boundary = bbox to ensure robust clipping
      net <- osmactive::get_travel_network(place = country_name, boundary = bbox)
      if (!is.null(net)) {
...
if (file.exists("outputs/city_lengths_all.csv")) {
  ...
  cols_to_add <- c("total_road_km", "off_road_km", "segregated_wide_km", "segregated_narrow_km", "painted_km", "shared_footway_km")
...
# Check for total_road_km to ensure we have the latest metrics
if ("total_road_km" %in% names(city_lengths_all)) {
  cities_done <- city_lengths_all |>
    filter(!is.na(total_road_km)) |>
    select(country, city_name)
} else {
...
plot_data_long <- plot_data |>
  ...
  filter(pct_network <= 50) # Filter extreme outliers

# Calculate R-squared values for facet labels
cor_data <- plot_data_long |>
  group_by(infra_type_label) |>
  summarise(
    r_squared = round(cor(pct_network, bike_share_pct, use = "complete.obs")^2, 3),
    n_cities = n(),
    label_text = paste0(infra_type_label, "\n(R² = ", r_squared, ")")
  )

# Join R2 labels back to plot data
plot_data_long <- plot_data_long |>
  left_join(cor_data, by = "infra_type_label")
...
# Extract for inline text
r2_vals <- setNames(cor_data$r_squared, cor_data$infra_type_label)

# Print summary table
cor_data |>
  rename(
    `Infrastructure Type` = infra_type_label,
    `R-squared` = r_squared,
    `Number of Cities` = n_cities
  ) |>
  knitr::kable(caption = "Correlation between cycling infrastructure types and cycling mode share.")
```

The analysis reveals varying degrees of association between infrastructure types and cycling levels when normalized by the total road network. Segregated tracks show a correlation of $R^2 =$ `r r2_vals["Segregated tracks"]`. Off-road paths ($R^2 =$ `r r2_vals["Off-road paths"]`) and painted lanes ($R^2 =$ `r r2_vals["Painted lanes"]`) show weaker associations.

## Discussion

The analysis reveals a clear hierarchy in the relationship between infrastructure types and cycling levels. **Segregated tracks** (both wide and narrow) demonstrate the strongest positive correlation with cycling mode share ($R^2 \approx 0.42$). This supports the hypothesis that physical separation from motor traffic is a key factor in enabling higher cycling levels.

In contrast, **painted lanes** show negligible correlation ($R^2 \approx 0.01$), suggesting that merely marking space on the carriageway without physical protection may not be sufficient to drive significant modal shift. **Off-road paths** and **shared footways** occupy a middle ground, contributing to the network but with weaker associations than dedicated segregated tracks.
